...
class DDPG(object):
    def __init__(self, state_size, action_size, actor_hidden_units = (300,600),
        ...
        """
        Contructs a DDPG Agent with the given parameters
        ...
        :param actor_hidden_units: Tuple(Int) denoting the actor's hidden layer
            ...
            network and the Int denotes the number of neurons in the later.
        ...
        :param critic_hidden_units: Tuple(Int) denoting the critic's hidden
            ...
            Critic network and the Int denotes the number of neurons in the layer.
        ...
        :param discount: Float denoting the discount(gamma) given to the future
    ...
    def train(self):
        ...
        Please note that the agent must have gone through more steps that the
    ...
    def _get_sample(self):
        """
        Finds a random sample of size self._batch_size from the agents's current
        memory.
    ...
    def _get_q_targets(self, next_states, done, rewards):
        ...
        unless there is not next state in which
        ...
        q_targets = [reward if this_done else reward + self._discount *
                     (next_q_value) for (reward, next_q_value, this_done)
    ...
    def _train_actor(self, states):
        ...
        :param states: List(List(Float)) denoting he states to train the Actor on
        :return: None
        """
    ...
    def _get_gradients(self, states):
        """ 
        Calculate the DDPG for actor training
    ...
    def remember(self, state, action, reward, done, next_state):
        """
        Stores the given stat, action, reward etc in the Agent's memory.
