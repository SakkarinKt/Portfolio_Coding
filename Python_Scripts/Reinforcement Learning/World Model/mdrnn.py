...
"""
Define MDNRNN model, supposed to be used as a world model on the latent space
"""
import argparse
...
def gmm_loss(batch, mus, sigmas, logpi, reduce = True):
    ...
    compute the minus the log probability of batch under the GMM model described
    by mus, sigmas, pi. Precisely, with bs1, bs2, ... the sizes of the batch
    dimensions (several batch dimension are useful when you have both a batch
    axis and a time step axis), gs the number of mixtures and fs the number of features.
...
class MDRNN(_MDRNNBase):
    ...
    def forward(self, actions, latents): # pylint: disable= arguments-differ
        ...
        :args latents: (SEQ_LEN, BSIZE, ASIZE) torch tensor
...
class MDRNNCell(_MDRNNBase):
    ...
    def forward(self, action, latent, hidden): # pylint: disable= argument - differ
        """ ONE STEP FORWARD.
        ...
        :returns: mu_nlat, sig_nlat, pi_nlat, r, d, next_hidden, parameters of the GMM
        prediction for the next latent, gaussian prediction of the reward and
        logit prediction of terminality and next hidden state
